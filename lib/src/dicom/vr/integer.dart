...
class VRInt extends VR<int> {
  ...
  bool isValidValue(int n) => min <= n && n <= max;

  String getValueError(int n) =>
      (!isValidValue(n)) ? 'Range Error: min($min) <= value($n) <= max($max)' : null;
  ...
  static const VRInt kUN = const VRInt._(05, 0x554e, "UN", "Unknown VR", 1, Uint8.min, Uint8.max, kMaxLongVF, true);
