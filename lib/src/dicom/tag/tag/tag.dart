...
class Tag {
  ...
  const Tag(this.code, [this.vr = VR.kUN, this.vm = VM.k1_n]);
...
// DICOM Attribute Definitions
 ...
 // final String name;
  ...
  //Fix or Flush
 Uint8List checkBytes(Tag tag, Uint8List bytes) => vr.checkBytes(bytes);
  ...
  bool isPrivateGroupCode(int tagCode) => Group.isPrivate(Group.fromTag(tagCode));

  /// Returns true if [code] is a valid [PrivateCreatorTag] tag.
  bool isPrivateCreatorCode(int tagCode) =>
      isPrivateGroupCode(tagCode) && Elt.isPrivateCreator(Elt.fromTag(tagCode));

  static bool isPrivateDataCode(int tag) =>
      Group.isPrivate(Group.fromTag(tag)) && Elt.isPrivateData(Elt.fromTag(tag));

  /// Returns true if [code] is a valid [PrivateCreatorTag] tag.
  bool isValidPrivateDataCode(int creatorCode, int dataCode) {
  ...
  static Tag lookup(dynamic intOrTag, [bool shouldThrow = true]) {
    if (intOrTag is Tag) return intOrTag;
    if (intOrTag is int) return PublicTag.lookup(intOrTag);
    if (intOrTag is! int && shouldThrow) tagError(intOrTag);
    return null;
  }
}
