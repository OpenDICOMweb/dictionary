...
void create() {
  test('photoem', () {
    var PhotometricInterpretation1 =
        const PhotometricInterpretation(1, "binayak", "42", const Term("abc", "hjgh"));
    expect(
    ...
    expect(
        PhotometricInterpretation1.lookup("RGB"), equals(PhotometricInterpretation.kRGB));
    ...
    expect(
        PhotometricInterpretation1.lookup("HSV"), equals(PhotometricInterpretation.kHSV));
  ...
  test("universal", () {
    var universal_entitytype1 =
        const UniversalEntityIDType(2, "binayak", "45", const Term("xya", "abc"));
    expect(
        universal_entitytype1,
        equals(
            const UniversalEntityIDType(2, "binayak", "45", const Term("xya", "abc"))));
    expect(UniversalEntityIDType.DNS,
        equals(const UniversalEntityIDType(0, "DNS", "Domain Name System", Term.DNS)));
    expect(universal_entitytype1.lookup("DNS"), equals(UniversalEntityIDType.DNS));
    expect(universal_entitytype1.lookup("EUI64"), equals(UniversalEntityIDType.EUI64));
    expect(universal_entitytype1.lookup("ISO"), equals(UniversalEntityIDType.ISO));
    expect(universal_entitytype1.lookup("URI"), equals(UniversalEntityIDType.URI));
    expect(universal_entitytype1.lookup("UUID"), equals(UniversalEntityIDType.UUID));
    expect(universal_entitytype1.lookup("X400"), equals(UniversalEntityIDType.X400));
    expect(universal_entitytype1.lookup("X500"), equals(UniversalEntityIDType.X500));
  });
}
...
void show1() {
  test("s", () {
    ...
    expect(
        ...
        equals(
            const ModalityType("AR", "Autorefraction", MClass.ACQUISITION, false, null)));
    expect(modalityclass,
        isNot(const ModalityType("AU", "Audio", MClass.ACQUISITION, false, null)));
...
//   var modalitytypes=const ModalityType(23,"binayak",const MClass(34),true,));
  ...
  test("colors", () {
    ...
    expect(color, equals(const ColorPalette("uid1", "label1", "dart language")));
  });
  test("yesorno", () {
    var yesnos = const YesNo(1, "yes", "Yes", const Term("abc", "abc"));
    expect(yesnos, equals(const YesNo(1, "yes", "Yes", const Term("abc", "abc"))));
